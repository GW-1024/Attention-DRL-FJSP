...
class FJSP(gym.Env, EzPickle):
    ...
    @override
    # --- 修改点 1: 参数名改为 gantt_chart ---
    def step(self, action, mch_a, gantt_chart=None):
        ...
        for i in range(self.batch_sie):
            ...
            if action[i] not in self.partial_sol_sequeence[i]:
                ...
                # --- 修改点 2: 恢复绘图逻辑并添加保护 ---
                # 仅对 Batch 中的第一个环境 (i==0) 进行绘图，防止重叠
                if gantt_chart is not None and i == 0:
                    # 确保传入的是标量数值
                    m_id = mch_a[i]
                    if isinstance(m_id, torch.Tensor):
                        m_id = m_id.item()
                    elif isinstance(m_id, np.ndarray):
                        m_id = m_id.item()
                        
                    gantt_chart.gantt_plt(row, col, m_id, startTime_a, self.dur_a,
                                          self.number_of_jobs)
                # ------------------------------------
...
class DFJSP_GANTT_CHART():
    ...
    def initialize_plt(self):
        # 根据机器数量动态调整画布高度，防止太挤
        fig_height = max(5, self.number_of_machines * 0.5) 
        plt.figure(figsize=(self.total_n_job, fig_height))
        
        # 1. 动态生成 Y 轴刻度 (从 1 到 机器总数)
        y_value = list(range(1, self.number_of_machines + 1))

        # 2. 改用英文标签，避免字体报错
        plt.xlabel('Time (Makespan)', size=14) 
        plt.ylabel('Machine ID', size=14)

        # 3. 设置刻度，移除 fontproperties 指定，使用默认字体
        plt.yticks(y_value, size=12)
        plt.xticks(size=12)
        plt.grid(axis='x', linestyle='--', alpha=0.5) # 加个网格线更好看
