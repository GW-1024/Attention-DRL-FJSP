    ...
    def update(self,  memories, epoch):
        ...
        for _ in range(configs.k_epochs):
            ...
            for i in range(len(memories.fea_mb)):
                ...
                a_index = memories.a_mb[i] # 这是一个索引, 不用动
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(data_loader):
            ...
            while True:
                ...
                # 2. 仅在调用模型前，才把数据发送到 GPU (注意变量名后缀是 _gpu)
                ...
                action, a_idx, log_a, action_node, _, mask_mch_action, hx = ppo.policy_old_job(x=env_fea_gpu,
                                                                                               graph_pool=g_pool_step,
                                                                                               padded_nei=None,
                                                                                               adj=env_adj_gpu,
                ...
                pi_mch,pool = ppo.policy_old_mch(action_node, hx, mask_mch_action, env_mch_time_gpu,mch_a,last_hh)
                # print(action,mch_a)
                mch_a, log_mch = select_action2(pi_mch)
                
                # 4. 把要存储的张量全部移回 CPU (使用 .cpu())
                job_log_prob.append(log_a.cpu())

                # print(action[0].item(),mch_a[0].item())
                mch_log_prob.append(log_mch.cpu())

                # 5. 在 memory 对象中，全部存储 CPU 上的张量 (_cpu)
            ...
            #filepath = 'my_trained_models'
            
            # --- 修改后的代码 (第 433 行) ---
            filepath = 'my_experiment_GNN'
